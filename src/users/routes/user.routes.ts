...
export class UserRoutes extends BaseRoutes<UserController, UserMiddleware> {
	...
	routes(): void {
		...
		//Update user
		this.router.put('/user', this.middleware.userAuth, this.middleware.validateUpdateDTO, (req, res) =>
		...
		//change password
		this.router.put('/change-password', this.middleware.userAuth, this.middleware.validateChangePasswordDTO, (req, res) =>
		...
		//Get user rank by id
		this.router.get('/user-rank/:id', this.middleware.userAuth, (req, res) => this.controller.getUserRankById(req, res));

		//Set points to user
		this.router.put('/set-points', this.middleware.userAuth, (req, res) => this.controller.setPointsUser(req, res));

		//Send email forgot password
		this.router.put('/forgot-password', this.middleware.validateEmailForgotPasswordDTO, (req, res) =>
			this.controller.sendEmailForgotPassword(req, res)
		);

		//Create new forgot password
		this.router.put('/reset-new-password', this.middleware.validateCreateNewForgotPasswordDTO, (req, res) =>
			this.controller.updateForgotPassword(req, res)
		);
